...
import com.everdata.command.Search.CommandArgv;
...
public class AST_Search extends SimpleNode {
	...
	*/
	...
	private static QueryBuilder genQueryBuilder(SimpleNode tree, String boostFields) throws CommandException{
		...
		case CommandParserTreeConstants.JJT_TERMEXPRESSION:
			AST_TermExpression t = (AST_TermExpression)tree;
			QueryBuilder rawQb = fromValueTypeQ("_all", t.term, t.type);
			if(boostFields != null)
				return boostWrapMatchAllWithFunctionScore(rawQb, t.term, boostFields);
			else
				return rawQb;
		case CommandParserTreeConstants.JJT_COMPARISONEXPRESSION:
			Expression expr = ((AST_ComparisonExpression)tree).expr;
		...
		case CommandParserTreeConstants.JJT_PREDICATEEXPRESSION:
			if(tree.children.length > 1){
				...
				for(Node n: tree.children){
					fb.must(genQueryBuilder((SimpleNode)n, boostFields));
				}
				
				return fb;
			}else
				return genQueryBuilder((SimpleNode)tree.children[0], boostFields);
		case CommandParserTreeConstants.JJT_OREXPR:			
			if(tree.children.length > 1){				
				BoolQueryBuilder fb = QueryBuilders.boolQuery();
				for(Node n: tree.children){
					fb.should(genQueryBuilder((SimpleNode)n, boostFields));
				}
		...
		case CommandParserTreeConstants.JJT_UNARYEXPR:			
			if(((AST_UnaryExpr)tree).isNot){
				BoolQueryBuilder fb = QueryBuilders.boolQuery().mustNot(genQueryBuilder((SimpleNode)tree.children[0], boostFields));
				return fb;
			}else
				return genQueryBuilder((SimpleNode)tree.children[0], boostFields);
	...
	private CommandArgv commandString = null;
	...
	public void setCommandArgv(CommandArgv cs){
		commandString = cs;
	}
	...
	private QueryBuilder getInternalQuery() throws CommandException {
		...
		for (Node n : children) {
			if (n instanceof AST_OrExpr) {
				allQuerys.add(genQueryBuilder((SimpleNode)n, commandString.boostFields));
				break;
			}
		}
		...
		if(childTypes != null){
			for(int i = 0; i< childTypes.size(); i++)
				allQuerys.add(QueryBuilders.hasChildQuery(childTypes.get(i), genQueryBuilder(childExpressions.get(i), commandString.boostFields)));
			
		}else if(parentType != null){
			
			allQuerys.add(QueryBuilders.hasParentQuery(parentType, genQueryBuilder(parentExpression, commandString.boostFields)));		
		
		}
		
		if(starttime == null)
			starttime = (String) getOption(Option.STARTTIME);
		if(endtime == null)
			endtime = (String) getOption(Option.ENDTIME);
